...
export class GitAnalyzer {
  ...
  public async getStagedChanges(): Promise<GitChangeInfo> {
    try {
      ...
      return {
        files: [...status.staged, ...status.modified, ...status.created],
      ...
      Logger.error(`Failed to get staged changes: ${(error as Error).message}`);
      throw error;
    }
  }
  ...
  public async getUnstagedChanges(): Promise<GitChangeInfo> {
    try {
      ...
      Logger.error(`Failed to get unstaged changes: ${(error as Error).message}`);
      throw error;
    }
  }
  ...
  public async getAllChanges(): Promise<GitChangeInfo> {
    try {
      ...
      Logger.error(`Failed to get all changes: ${(error as Error).message}`);
      throw error;
    }
  }

  public async getBranchDiff(targetBranch: string = 'main'): Promise<GitChangeInfo> {
    try {
      const status = await this.git.status();
      const currentBranch = status.current || 'HEAD';
      
      // Get diff between current branch and target
      const diff = await this.git.diff([`${targetBranch}...${currentBranch}`]);
      const diffStat = await this.git.diffSummary([`${targetBranch}...${currentBranch}`]);
      
      // Get list of changed files
      const changedFiles = await this.git.diff([`${targetBranch}...${currentBranch}`, '--name-only']);
      ...
      Logger.error(`Failed to get branch diff: ${(error as Error).message}`);
      throw error;
    }
  }
  ...
  public async getCurrentBranch(): Promise<string> {
    try {
      ...
      Logger.error(`Failed to get current branch: ${(error as Error).message}`);
      throw error;
    }
  }
  ...
  public async getRecentCommits(count: number = 5): Promise<string[]> {
    try {
      ...
      return log.all.map(commit => `${commit.hash.substring(0, 7)}: ${commit.message}`);
    } catch (error) {
      Logger.error(`Failed to get recent commits: ${(error as Error).message}`);
      throw error;
    }
  }
  ...
  public async hasChanges(): Promise<boolean> {
    try {
      ...
      Logger.error(`Failed to check for changes: ${(error as Error).message}`);
      return false;
    }
  }
  ...
  public async hasStagedChanges(): Promise<boolean> {
    try {
      ...
      Logger.error(`Failed to check for staged changes: ${(error as Error).message}`);
      return false;
    }
  }
  ...
  public async stageAll(): Promise<void> {
    try {
      ...
      Logger.error(`Failed to stage changes: ${(error as Error).message}`);
      throw error;
    }
  }
  ...
  public async getRemoteBranches(): Promise<string[]> {
    try {
      ...
      return Object.keys(branches.branches).filter(branch => 
        branch.startsWith('origin/') && !branch.includes('HEAD')
      ).map(branch => branch.replace('origin/', ''));
    } catch (error) {
      Logger.error(`Failed to get remote branches: ${(error as Error).message}`);
      return [];
    }
  }
  ...
  public async getLocalBranches(): Promise<string[]> {
    try {
      ...
      Logger.error(`Failed to get local branches: ${(error as Error).message}`);
      return [];
    }
  }
